...
contract Campaign is ERC721, ERC721URIStorage, ERC721Pausable, Ownable, ERC721Burnable {
    ...
    event SubmissionCreated(address indexed userId, string imageUrlOrHash, string description, uint256 submissionId);
    ...
    function createSubmission(string memory _imageUrlOrHash, string memory _description) external {
        ...
        emit SubmissionCreated(msg.sender, _imageUrlOrHash, _description, submissions.length);
    }
    ...
    function parseJudgingType(string memory _judgingType) internal pure returns (JudgingType) {
        if (keccak256(abi.encodePacked(_judgingType)) == keccak256(abi.encodePacked("Admin"))) {
            return JudgingType.Admin;
        } else if (keccak256(abi.encodePacked(_judgingType)) == keccak256(abi.encodePacked("Public"))) {
    ...
    function upvoteSubmission(address voter, uint256 _submissionId) external {
        ...
        require(!hasVoted[_submissionId][voter], "User has already upvoted this submission");
    ...
    function calculateWinners() external {
        ...
        uint256 maxWinners = totalWinners > submissions.length ? submissions.length : totalWinners;
